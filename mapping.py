...
def static_mapping(cg, voxel_field, mlp, dataloder, sampler, startframe, endframe):
    ...
    # ###########################################################
    # save_dir = cg.output_root
    # os.makedirs(save_dir, exist_ok=True)
    # save_path = os.path.join(save_dir, f"model_{startframe:06d}-{endframe-1:06d}.pth")

    # checkpoint = {
    #     "voxel_field": voxel_field.state_dict(),          
    #     "mlp": mlp.state_dict(),                          
    #     "feature_indexs_list": [idx.clone().cpu() for idx in voxel_field.feature_indexs_list],
    #     "config": cg.__dict__,                       
    # }
